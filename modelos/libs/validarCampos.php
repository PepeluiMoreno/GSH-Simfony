...
/*--------------------- Inicio validarEmail ------------------------------------
2019-12-11
Entre otros caracteres especiales tampoco acepta letras acentuadas y ñ,Ç 

2022-05_19: Agustín 
.Anulo filter_var($email,FILTER_SANITIZE_EMAIL); mejor que el usuario corrija despues de aviso de error.
.Anulo filtrar con !preg_match("/^[a-zA-Z0-9\+]+([._-][a-zA-Z0-9\+]+)*@([a-zA-Z0-9]+([._-][a-zA-Z0-9]+))+$/",$email)
No dejaba pasar un dominio de un socio con guiones pero existente.
.Dejo filtro filter_var($email,FILTER_VALIDATE_EMAIL) cosidero que es un estandar de validación
.Añado if(checkdnsrr($arrEmailSeparado[$ultimoElemento],"MX"))
Que comprueba si ese dominio existe un registro MX en sus DNS, lo cual además implica que el formato será válido.
------------------------------------------------------------------------------*/
...
{ 
  ...
  $resulValidarCampo = validarCampoNoVacio($email, $textoErrorCampo);//pone el mensaje de vacío, si es el caso 
		...
		if ($resulValidarCampo['codError'] == '00000')
		{ 
	   $email = trim($email);//Se quitan espacios de inicio o final, echo "UTF-8 nombre:",$telefono,"<br>"; 			
    
				//$email = filter_var($email,FILTER_SANITIZE_EMAIL); //No usar: es mejor que corrija el email el propio socio
				
    //echo "<br /><br />2 validarEmail:email: $email<br />";			

			if (!filter_var($email,FILTER_VALIDATE_EMAIL) ) 
			{ 
					//echo "<br /><br />3 validarEmail:email: $email<br />";
			...
			{				
				$arrEmailSeparado = explode("@",$email); 
				
				//echo "<br /><br />4 validarEmail:arrEmailSeparado: "; print_r($arrEmailSeparado);	

				$dominio = $arrEmailSeparado[array_key_last($arrEmailSeparado)];    
   	
    //echo "<br /><br />5 validarEmail:dominio:  $dominio";						

    if(!checkdnsrr($dominio,"MX"))//chequea si ese dominio existe un registro MX en los DNS, lo cual además implica que el formato será válido.				
				{
      //echo "<br /><br />6 validarEmail:no existe el dominio:  $dominio";			
						$resulValidarCampo['codError'] = '81010';
						$resulValidarCampo['errorMensaje'] = " El dominio no es válido o no existe: ".$dominio;							
				}
			}		
	 }
	$resulValidarCampo['valorCampo'] = $email;
	
	//echo "<br /><br />8 validarEmail:resulValidarCampo: ";print_r($resulValidarCampo);	
