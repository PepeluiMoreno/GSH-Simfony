...
/*--------- Inicio exportarCuotasXMLBancos_old --------------------------------------------------------
...
---------------------------------------------------------------------------------------------------*/
function exportarCuotasXMLBancos_old($codAreaCoordinacion,$condicionEmail,$arrayFormDatosElegidos)//2020_11_19_BIC_ES_GENERICO_SEPA_LISTAERROR
{
...
/*----------- Fin exportarCuotasXMLBancos_old ---------------------------------------------------------*/
...
NOTA: 2020-11-19: SEGUN B.SANTANDER AHORA YA NO SERÍA NECESARIO BIC PARA "ES" PERO SI PARA OTROS 
PAISES SEPA	ADEMAS DICEN QUE MEJOR SIEMPRE "RCUR"	
2024-10-15: Correciones necesarias por nueva versión "10.3.39-MariaDB-0+deb10u2" que daba ERROR
---------------------------------------------------------------------------------------------------*/
				...
				{/*
					...
					*/ 				
					...
					{					
						...
						/*Desde versión "10.3.39-MariaDB-0+deb10u2" da ERROR en insertar valor $fechaCreacion=date("c") en tabla REMESAS_SEPAXML.FECHA_CREACION_ARCHIVO_SEPA (Tipo Índice 	datetime)
						  Error: $fechaCreacion = date("c");//generó 2024-10-14T09:41:29+02:00, "c" Fecha ISO8601 (añadido en PHP 5), Error: insertar tabla REMESAS_SEPAXML.FECHA_CREACION_ARCHIVO_SEPA (Índice datetime) 					         
						  SEPA_ISO20022 1.2 REQUIERE Fecha y hora de creación- CreationDateTime – dice <CreDtTm>2010-11-05T09:25:15</CreDtTm> debe ser YYYY-MM-DDThh:mm:ss (longitud 19) ejemplo 2010-11-05T09:25:15
		    */
					 $fechaCreacion = (new DateTime())->format('Y-m-d\TH:i:s');//generó 2024-10-12T11:05:46; valido para "insertarOrdenesCobroRemesa()" en tabla REMESAS_SEPAXML.FECHA_CREACION_ARCHIVO_SEPA (Índice tipo	datetime) lo guarda como 2024-10-12 11:05:46
      //echo "<br><br>3-5 modeloTesorero:exportarCuotasXMLBancos:fechaCreacion: ";print_r($fechaCreacion); 
						
					/*"$fechaOrdenCobroRemesa" viene del formulario en $arrayFormDatosElegidos y lo elige tesorería.								
					  SEPA_ISO20022 2.18 Fecha de cobro – RequestedCollectionDate: dice  Fecha de cobro <ReqdColltnDt> longitud 10;	Etiqueta XML: <ReqdColltnDt> Ocurrencias: [1..1] Formato: ISODate YYYY-MM-DD			
					  Error anterior: $fechaOrdenCobroRemesa=$arrayFormDatosElegidos['fechacobro']['anio']."-".$arrayFormDatosElegidos['fechacobro']['mes']."-".$arrayFormDatosElegidos['fechacobro']['dia'].date("TH:i:s");
					  Anterior línea da problemas en tabla REMESAS_SEPAXML.FECHAORDENCOBRO "tipo 	varchar(10)", ya que es sería más longitud de 10
				 */
      $fechaOrdenCobroRemesa = $arrayFormDatosElegidos['fechacobro']['anio']."-".$arrayFormDatosElegidos['fechacobro']['mes']."-".$arrayFormDatosElegidos['fechacobro']['dia'];//2024-10-12 vale para tabla REMESAS_SEPAXML						
						
						//echo "<br><br>3-6 modeloTesorero:exportarCuotasXMLBancos:fechaOrdenCobroRemesa: ";print_r($fechaOrdenCobroRemesa);		
	     //$nombreArchivoSEPA = $arrayFormDatosElegidos['nombreArchivoSEPA']; //será: $nombreArchivoSEPA = "SEPA_ISO20022CORE" y viene del formulario				

						/*"$nombreArchivoSEPAXML", será la clave primaria en "REMESAS_SEPAXML" y también forma parte de la PK "ORDENES_COBRO", 
						  
								Se compone de dos partes "$nombreArchivoSEPA" = $arrayFormDatosElegidos['nombreArchivoSEPA']; siendo $nombreArchivoSEPA = "SEPA_ISO20022CORE" 
								y viene del formulario	la segunda parte con la fecha del día de orden de cobro que viene del formulario, en el campo $arrayFormDatosElegidos['fechacobro'], 
								pero se añade date("TH:m:s"), que recoge el momento de creación del archivo SEPAXML hasta los segundos, 
								con lo cual podría haber dos órdenes de cobro para el mismo día, pero será muy improbable que coincidan que con la misma hora+minutos+segundos 
								y riesgo mínimo de duplicidad de clave primaria.
								Esta repetición está tratada en los errores y aconseja repetir la operación pasado un tiempo
								Nota: insertarOrdenesCobroRemesa() controla parámetros recibidos, contiene "insertarError()" y retorna mensaje error			
						*/						
					
						//$nombreArchivoSEPAXML = $nombreArchivoSEPA."_".date_format(new DateTime($fechaOrdenCobroRemesa),'Y_m_d\HH_i_s').".xml";//genera: SEPA_ISO20022CORE_2024_10_18H00_00_00.xml nota siempre 00-00-00 no sirve 

						$nombreArchivoSEPAXML = $nombreArchivoSEPA."_".date_format(new DateTime($fechaOrdenCobroRemesa),'Y_m_d').(new DateTime())->format('\HH_i_s').".xml";
						//genera:SEPA_ISO20022CORE_2024_10_18H19_42_30.xml; que es fecha orden cobro (YYYY-MM-DD)+(HH-m-s) del momento de la creación. 
						//echo "<br><br>4-1 modeloTesorero:exportarCuotasXMLBancos:nombreArchivoSEPAXML: ";print_r($nombreArchivoSEPAXML);				
	        ...
	        $textoErrores = " ERROR: En insertarOrdenesCobroRemesa(). CODERROR: ".$resInsertarOrdenesCobroRemesa['codError'].$resInsertarOrdenesCobroRemesa['errorMensaje'];					 									
							...
							{ 
									...
									*/	
									...
									$resSEPA_XML_SDD_Cuotas = SEPA_XML_SDD_CuotasTesoreroSantanderWrite($arrDatosCobroCuotas,$arrayFormDatosElegidos,$fechaCreacion);//en modeloTesorero.php	probado error			
									
									//echo "<br><br>5-1 modeloTesorero:exportarCuotasXMLBancos:resSEPA_XML_SDD_Cuotas: ";print_r($resSEPA_XML_SDD_Cuotas);
