...
DESCRIPCION: Es el modelo base de las funciones de MySQL.
             Mayormente llamadas desde los modelos "modelos" de la aplicación, aunque también
													podría ser llamada desde otros lugares.
													En este script están las funciones base de consultas, inserciones, actualizaciones
													y borrado de la BBDD MySQL.
													
													Es una adaptación del modelo utilizado en las versiones anteriores de php, para 
													adaptarlo a las nuevas versiones de PHP 7.x que es necesario utilizar PDO.
...
OBSERVACIONES: Necesita previa conexión a la BBDD que se hace previmente en modelos 
               (o acaso en algún controlador), y que se recibe como parámetro.
															No conviene hacer la llamada a la función de conexión aquí desde esta funciones 
															por posibles problemas con el rollback	de las transaciones.
															
															
***NOTA: A fecha 2021-12-07, la versión del servidor de Nodo50, es 10.1.48-MariaDB-0+deb9u2 Inicialmente fue la MySQL 5.7

MySQL y MariaDB, EN LA CONFIGURACIÓN POR DEFECTO que procede de la migración de MySQL y en insert y updates convierte 
tipos de forma para mi gusto demasiado flexible, por ejemplo:

INSERT y UPDATE, Si $contenido = "" o "letras" y tipo columna es número int pone 0(cero), o si es decimal(10,2) pone 0.00; 
SELECT * FROM PAIS_pruebas WHERE CODPAIS3 ='texto' si la columna CODPAIS3 fuese tipo int en la tabla el string 'texto' lo convierte a 0

Hasta ahora, en esta aplicación los controles previos a esas operaciones evitan esas no deseadas conversiones.
Pero sería mejor, cambiar a sql_mode = 'STRICT_TRANS_TABLES', o mejor a 'STRICT_ALL_TABLES', para que diese un error en 
caso de conversiones de esos tipo, pero para eso habría que probar previamente las posibles implicaciones.
...
Se puede cambiar tambien temporalmete en la sesión.

SET sql_mode = 'STRICT_TRANS_TABLES'; evita conversiones	de tipo tablas InnoDB engine
SET sql_mode = 'STRICT_ALL_TABLES';	este mejor evita conversiones	de tipo por ejemplo todas tipos engine

MariaDB tengo version 10.1.48-MariaDB-0+deb9u2 ver buenos ejemplos: (https://mariadb.com/kb/en/sql-mode/#strict-mode)

otro: https://imperioweb.net/como-desactivar-el-modo-estricto-o-strict-mode-de-mysql-y-mariadb
																
------------------------------------------------------------------------------------------------*/

function normalizarUtf8($valor)
{
	if (!is_string($valor)) {
		return $valor;
	}

	// Avoid double-encoding strings already in UTF-8.
...
En esta función ejecuta una orden SQL: Sirve para SELECT, INSER, UPDATE,DELETE.

Dado que ya existen otras funciones específicas para cada uno de esos casos, esta función la utilizo
opcionalmente normalmente para órdenes select más complejas.					
...
DEVUELVE: un array con el resultado y el número de filas implicadas (que puede ser 0, 
          sin que sea un error, y los códigos de errores
								
LLAMADA: modeloAdmin.php:eliminarDatosSocioBajas5Anios(), para update con subselect 
modeloAdmin.php:insertarImporteCuotasAnioSiguiente() para insert con select y normalmente
para otros casos complejos
 ...
 $arrResultado['errorMensaje'] = '';
	...
	try {		
			...
			{					
					...
					{						
								$resulFilasAux[$f][$indice] = normalizarUtf8($contenido);
					}
					$f++;
			}
...
function insertarUnaFila($tablaInsercion,$arrayCamposValoresInsercion,$conexionLinkDB) {
 ...
 //echo '<br><br>0-1 modeloMySQL.php:insertarUnaFila:arrayCamposValoresInsercion: ';print_r($arrayCamposValoresInsercion); 
	...
	try {	
			...
			foreach ($arrayCamposValoresInsercion  as $indice =>$contenido)//foreach para formar $cadSql con PHP: PDOStatement::bindParamValue
			{
				$cadenaCamposInsercion     = $cadenaCamposInsercion.','.$indice;
				$bindCadenaCamposInsercion = $bindCadenaCamposInsercion.', :'.$indice;	
			
				if (isset($contenido) )// if para que no de Notice la siguiente. OJO !empty($contenido) no es adecuado aquí pues el 0 (cero) y "0" lo trata como vacío y pondría null
				{	
				  $arrBindValues[':'.$indice] = utf8_decode($contenido);	
				}
				else
				{	
						$arrBindValues[':'.$indice] = $contenido;//Creo que mejor opción: si $contenido no tiene valor (no existe) pone NULL, o tiene valor NULL pone NULL; (para número y char)
				}					
			}
			...
			//echo '<br><br>1-3 modeloMySQL.php:insertarUnaFila:cadenaCamposInsercion: ';print_r($cadenaCamposInsercion); 
			//echo '<br><br>1-4 modeloMySQL.php:insertarUnaFila:bindCadenaCamposInsercion: ';print_r($bindCadenaCamposInsercion); echo '<br />';
			//echo '<br><br>1-5 modeloMySQL.php:insertarUnaFila:arrBindValues: ';print_r($arrBindValues); echo '<br />';						
...
RECIBE: "$tablaBorrar" recibida como string, tabla de la que se borran las filas,
        ...
        "$cadenaCondiciones" que es un string, 
								 por ejemplo: $cadenaCondiciones = "CODUSER :codUser" o "CODUSER = :codUser AND CODROL = :codRol"
...
RECIBE: "$tablaActualizar" (string), 
        "$arrayCondiciones" para el WHERE, ejemplo: 
											$arrayCondiciones['CODUSER']['valorCampo'] = 83;
											$arrayCondiciones['CODUSER']['operador'] = '='; (podrían ser otros <=, !=, ...)
											$arrayCondiciones['CODUSER']['opUnir'] = ' ';//podría ser AND, OR, ''	última para condición	
         
        "$arrayCamposValoresActualizar" array con campos de la tabla y valores	de los campos a actualizar, ejemplo:							
											$arrayCamposValoresActualizar['APE1'] = 'cosgaya';
											$arrayCamposValoresActualizar['APE2'] = 'ayuela';
											$arrayCamposValoresActualizar['NOM'] = 'agustin';								
						
        "$conexionLinkDB" un link de conexión PDO a la BBDD
							
							Ejemplo:	UPDATE MIEMBRO 	SET APE1 = :APE1, APE2 = :APE2, NOM =, :NOM (los campos a actulizar)  
																WHERE CODUSER = :CODUSER (las condiciones)		
																
							$arrBindValues([:APE1]=>cosgaya, [:APE2]=>ayuela, [:NOM]=>agustin), [:CODUSER]=>83) (valores para sustituir)	
...
ACLARACIÓN:
arrResultado['numFilas'] = $sentencia->rowCount() puede ser "0", cuando no se cumplan las 
   ...
   /*Ejemplo:	UPDATE MIEMBRO SET APE1 ='cosgaya', APE2 ='ayuela',  NOM = agustin' (son los campos a actualizar)  
			...
			*/ 
			...
			{    			 									
					...
					{					
						 $arrBindValues[':'.$indice] = $contenido;				      				
					}
			} 
    ...
    NOTA: aquí SÍ se podría producir un problema si se repitiese el mismo campo en campos a actualizar y en condiciones (por ejemplo añadir la condición APE1 = :APE1 daría error).	
			*/ 
			...
			foreach ($arrayCondiciones  as $indice =>$contenido) //añadir if por si es contenido null y quitar campo??
			{ 		  				
...
/*--------------------------- Fin actualizarTabla con $arrBindValues -----------------------------*/
...
Se obtiene la cadena de "Campos a Actualizar" y cadena "Campos de Condiciones" parametrizadas por posición con "?"
También se prepara el array "$arrCamposPosicionValues" los con valores posicionales. 

Controla excepciones PDOException, y Exception.

RECIBE: "$tablaActualizar" (string), 
        "$arrayCondiciones" para el WHERE, ejemplo: 
           $arrayCondiciones:Array([ANIOCUOTA]=>Array([valorCampo]=>2020 [operador]=> = [opUnir]=>AND )
                                   [CODSOCIO]=>Array([valorCampo]=>83 [operador]=> = [opUnir]=>" " )) 
																																			
        "$arrayCamposValoresActualizar" array de campos con tabla y valores valores de campos a actualizar, ejemplo:								
           $arrayCamposValoresActualizar:Array([ANIOCUOTA]=>2020 [CODCUOTA]=>General [IMPORTECUOTAANIOSOCIO]=>50.20 ) 								
								
        "$conexionLinkDB" un link de conexión PDO a la BBDD
								
Ejemplo: UPDATE CUOTAANIOSOCIO SET ANIOCUOTA = ?,CODCUOTA = ?,IMPORTECUOTAANIOSOCIO = ?
						 		WHERE ANIOCUOTA = ? AND CODSOCIO = ?
									
							 	$arrPosicionValues([2020],['General'],[50.20],[2020],[83])(se sustituyen en cada "?" en el mismo orden)								
  ...
  //echo '<br><br>0-1 modeloMySQL.php:actualizarTabla_ParamPosicion:arrayCondiciones: ';print_r($arrayCondiciones);	
	...
	try {		
     ...
     $cadenaCamposPosicionActualizar .= ','.$indice. '=  ?';							
				   ...
				   $arrCamposPosicionValues[] = $contenido;	
					}
			}			
			$cadenaCamposPosicionActualizar = ltrim($cadenaCamposPosicionActualizar, ",");
			
			
			//echo '<br><br>2-1 modeloMySQL.php:actualizarTabla_ParamPosicion:$cadenaCamposPosicionActualizar:',$cadenaCamposPosicionActualizar; 
			//echo '<br><br>2-2 modeloMySQL.php:actualizarTabla_ParamPosicion:arrCamposPosicionValues: ';print_r($arrCamposPosicionValues);
    ...
    NOTA: aquí NO se  producirán problemas si se repitiese el mismo campo en campos a actualizar y en condiciones.	
			*/	
			...
			foreach ($arrayCondiciones  as $indice =>$contenido) //?????acaso aquí también if para valores nulos
			{ 						
     ...
     $arrCamposPosicionValues[] = $contenido['valorCampo'];//se añaden los valores de los condiciones a los ya existente en array $arrCamposPosicionValues[]	
			...
			//echo '<br><br>2-3 modeloMySQL.php:actualizarTabla_ParamPosicion:cadenaCondicionesPosicion: ';print_r($cadenaCondicionesPosicion);
			//echo '<br><br>2-4 modeloMySQL.php:actualizarTabla_ParamPosicion:$arrCamposPosicionValues: ';print_r($arrCamposPosicionValues);
			
			$cadSql = "UPDATE $tablaActualizar SET $cadenaCamposPosicionActualizar WHERE $cadenaCondicionesPosicion";
																														
			//echo '<br>3 modeloMySQL.php:actualizarTabla_ParamPosicion:cadSql: '.$cadSql;
			
			$sentencia = $conexionLinkDB->prepare($cadSql);// Se crea un objeto PDOStatement.
			
			//echo '<br><br>4-1 modeloMySQL.php:actualizarTabla_ParamPosicion:sentencia: ';print_r($sentencia);
			...
			//echo '<br><br>4-2 modeloMySQL.php:actualizarTabla_ParamPosicion: sentencia: ';print_r($sentencia);				
			//echo '<br><br>4-3 modeloMySQL.php:actualizarTabla_ParamPosicion:pdo-número de registros: '.$sentencia->rowCount();//Esta función va bien para INSERT, UPDATE, DELETE, pero puede con SELECT puede no ser correcta para alguna BBDD
			...
			$sentencia->closeCursor(); // Se libera el recurso.
			 ...
			 $arrResultado['errorMensaje'] = 'Error PDO en el sistema al actualizar filas en tabla: '.$tablaActualizar.', codError:'.$arrResultado['codError'].
				...
				//echo "<br><br>5 modeloMySQL.php:actualizarTabla_ParamPosicion:errorMensaje: ".$arrResultado['errorMensaje'];
				//echo "<br><br>5b modeloMySQL.php:actualizarTabla_ParamPosicion:Cadena informativa de la excepción:__toString(): ".$e->__toString()."<br>";//Puede que demasiada información en la cadena								
	}
	catch (Exception $e) {
			 ...
			 $arrResultado['errorMensaje'] = 'Error NO PDO en el sistema al actualizar filas en tabla: '.$tablaActualizar.', codError:'.$arrResultado['codError'].
				...
				//echo "<br><br>6-1 modeloMySQL.php:actualizarTabla_ParamPosicion:errorMensaje: ".$arrResultado['errorMensaje'];
				//echo "<br><br>6-2 modeloMySQL.php:actualizarTabla_ParamPosicion:Cadena informativa de la excepción:__toString(): ".$e->__toString()."<br>";//Puede que demasiada información en la cadena
	}	
...
/*--------------------------- Fin actualizarTabla_ParamPosicion ----------------------------------*/
...
//----------------------------- Inicio actualizarTabla_pdo_sin_bind ------------------
/* actualizarTabla_pdo_sin_bind YA NO USO
...
*/
//-------------------------------------------------------------------------
function actualizarTabla_pdo_sin_bind_YaNoUso ($tablaActualizar,$arrayCondiciones,$arrayCamposValoresActualizar,$conexionLinkDB)
{
	echo '<br><br>0-1 modeloMySQL.php:actualizarTabla_pdo_sin_bind:arrayCondiciones: ';print_r($arrayCondiciones);
...
//--------------------------- Fin actualizarTabla_pdo_sin_bind --------------
