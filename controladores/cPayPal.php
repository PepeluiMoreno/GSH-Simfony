...
Como aún no ha confimado su alta no se actualiza la tabla "CUOTAANIOSOCIO" con ese pago, 
tampoco la tabla "SOCIOSCONFIRMAR".
...
LLAMADO: En el proceso inicial de altaSocio (controladorSocios:altaSocio(), 
         al final en la pantalla con información, también aparece la CC de EL y 
									un botón con un link a pagar cuota en PayPal (opcional). 
         Esto es en la fase inicial de registrarse antes de confirmar el alta por el socio.  
         Si paga en ese momento con PayPal si el pago por PayPal que haya sido correcto, 
									y después pulse en PayPal el link volvera a Europa Laica, a esta función 
									cPayPal:confirmadoPagoAltaSocioPayPal_Registrarse() 
...
Ver OBSERVACIONES:  
LLAMADO: Desde el link del texto que se pone en PayPal, después de aceptar un pago por PayPal, que podría venir de: 
									- el menú de socio "Pagar cuota anual",
									- desde el link en el email enviado por tesorería para avisar de cuota no aún no pagada a socios sin CC 
									  cTesorero:emailAvisarCuotaNoCobradaSinCCemailAvisarCuotaNoCobradaSinCC()
         - al final de la operación de confirmar alta socio por el socio: controladorSocios:confirmarAltaSocio()											
...
- Pega: El problema de salir el socio cerrando la ventana de PayPal, en lugar de pulsar el link, 
  ...
  necesita un listener, en Nodo50, para escuchar los sucesivos envíos desde PayPal,
		hasta que se contesta que ha sido recibido.	
		2025-02-13: correción en línea: actualizCuotaAnioSocio('CUOTAANIOSOCIO',$codSocio,$arrayDatosAct,"")
--------------------------------------------------------------------------------------------------*/
   ...
   //echo"<br><br>2-a1 cPayPal:confirmarPagoPayPal:reDatosSocio:";print_r($reDatosSocio);	echo"<br><br>";		
    ...
    {					
				 $arrayDatosAct['IMPORTEGASTOSABONOCUOTA']['valorCampo'] = $reDatosSocio['valoresCampos']['datosFormCuotaSocio'][date('Y')]['IMPORTEGASTOSABONOCUOTA']['valorCampo'] + 
					                                                          //$datosPagoPayPal['gastosPayPal'];	//incluye EUR y da error SQL
																																																															$_POST['mc_fee'];
				}
				else
				{ //$arrayDatosAct['IMPORTEGASTOSABONOCUOTA']['valorCampo'] = $datosPagoPayPal['gastosPayPal'];// incluye EUR y da error SQL
			   $arrayDatosAct['IMPORTEGASTOSABONOCUOTA']['valorCampo'] = $_POST['mc_fee'];
				}	
    //--- Fin  Nuevo por si un socio modifica la cantidad y paga menos que IMPORTECUOTAANIOEL ------
 			...
 			$arrayDatosAct['OBSERVACIONES']['valorCampo'] = $reDatosSocio['valoresCampos']['datosFormCuotaSocio'][date('Y')]['OBSERVACIONES']['valorCampo'].". Fecha pago PayPal: ".$datosPagoPayPal['fechaPago'].
				                                                " Id. transación: ".$datosPagoPayPal['IdentificadorTransaccion']. ". Pagada en moneda: ".$_POST['mc_currency'].
				...
				require_once './modelos/modeloSocios.php';					
				//$resActualizCuotaAnioSocio = actualizCuotaAnioSocio('CUOTAANIOSOCIO',$codSocio,$arrayDatosAct); //antiguo se sustituye para parametro $conexionLinkDB =""
				//$resActualizCuotaAnioSocio = actualizCuotaAnioSocio('CUOTAANIOSOCIO',$codSocio,$arrayDatosAct,$conexionLinkDB); 
