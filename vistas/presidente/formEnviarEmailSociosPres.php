<?php
/*---------------------------------------------------------------------------------------------------------------
FICHERO: formEnviarEmailSociosPres.php
PROYECTO: EL
VERSION: PHP 7.3.21

DESCRIPCION:													
Formulario de selección para formar los emails a enviar a los socios desde presidencia, que permite seleccionar
los emails de los socios destinatarios por:	CODAGRUPACION,CODPAISDOM, CCAA, CODPROV.
...
LLAMADA: vistas/presidente/vCuerpoEnviarEmailSociosPres.php y antes desde cPresidente.php:enviarEmailSociosPres()	

OBSERVACIONES: Sustituyo CC por BCC
...
<div id="registro">
 ...
 <form name="registrarSimp" method="post"
       action="./index.php?controlador=cPresidente&amp;accion=enviarEmailSociosPres" enctype="multipart/form-data">
   ...
   <!-- ********** Inicio Selección de remitente (Se podrían añadir más remitentes para elegir) ******* -->							
    <label>*<b>Elegir remitente (desde, from, ...)</b></label>	
	    ...
	    <input type="radio"
	           ...
	           value='presidencia@europalaica.org'
						 <?php if (isset($datosEmail['camposEmail']['FROM']['valorCampo']) && $datosEmail['camposEmail']['FROM']['valorCampo']=='presidencia@europalaica.org')
	           {  echo " checked";}
	           ?>						 
	    /><label>presidencia@europalaica.org</label>	
					
	    <input type="radio"
	           name="datosEmail[camposEmail][FROM]"
	           value='vicepresidencia@europalaica.org'
						 <?php if (isset($datosEmail['camposEmail']['FROM']['valorCampo']) && $datosEmail['camposEmail']['FROM']['valorCampo']=='vicepresidencia@europalaica.org')
	           {  echo " checked";}
	           ?>						 
	    /><label>vicepresidencia@europalaica.org</label>								
					
	    <input type="radio"
	           name="datosEmail[camposEmail][FROM]"
	           value='secretaria@europalaica.org' 
						 <?php if (isset($datosEmail['camposEmail']['FROM']['valorCampo']) && $datosEmail['camposEmail']['FROM']['valorCampo']=='secretaria@europalaica.org')
	           {  echo " checked";}
	           ?>
	    /><label>secretaria@europalaica.org</label>					
		   <br /><br />		
