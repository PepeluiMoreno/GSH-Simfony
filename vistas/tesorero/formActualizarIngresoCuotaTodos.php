<?php
/*--------------------------------------------------------------------------------------------------
FICHERO: formActualizarIngresoCuotaTodos.php
VERSION: PHP 7.3.21
...
OBSERVACIONES:
El formulario alternativo es B- "formActualizarIngresoCuotaObservaciones.php", para pendientes de cobro 
de remesa en el banco, solo se permite cambiar los campos Observaciones y Motivo devolución.
--------------------------------------------------------------------------------------------------*/
...
<div id="registro">
 ...
 <!--	Inicio Estado actual de pago de la cuota a ------------------------>			
  ...
  />	
		...
		<label>Cambiar estado actual de la cuota <?php echo $datSocio['formIngresoCuota']['ANIOCUOTA']['valorCampo'] ?></label>
		<?php	
   $parValorEstadoCuota = array("ABONADA"=>"Abonada",
																												   	"ABONADA-PARTE"=>"Abonada en parte"
																																/*,
																												 	  "NOABONADA-DEVUELTA"=>"Devuelta, no abonada" 
																																,"NOABONADA-ERROR-CUENTA"=>"Error cuenta. O J O:se borrarán datos bancarios" se pondrá como "NOABONADA-DEVUELTA"*/
																													   );	
				...
				{	
				 $parValorEstadoCuota["NOABONADA-DEVUELTA"] = "Devuelta, no abonada";
				}	
    ...
    if ($datSocio['formIngresoCuota']['IMPORTECUOTAANIOEL']['valorCampo'] == 0)				
     ...
     unset ($parValorEstadoCuota["NOABONADA-DEVUELTA"]);					
				}
  ...
  <!--	Fin Estado actual de pago de la cuota a -------------------------->		
	 ...
	 <span class="error">
							...
							)
				{
						...
						" ***<br /><br />No modificar datos bancarios y de cuotas hasta después del abono por el banco</strong><br /><br />";
				}
				?>
		</span> 		
  ...
  <!-- Inicio Total bruto pagado  -------------------------------->
    ...
    echo "<label>GASTOS TOTALES, si los hubiese, al abonar y devolver la cuota  (los cobrados por PayPal, transferencia  o la entidad bancaria)</label>";
		...
		<br />		
		 <label> <strong>Añadir más GASTOS AL ABONAR LA CUOTA</strong> producidos al pagar mediante PayPal, transferencia, cheques</label> 
   ...
   <label>Nota:- Si se necesitase corregir un error previo de anotación en la cantidad anotada en gastos de ABONO DE CUOTA, para disminuir poner signo menos delante (-) delante de la cantidad a restar</label>
		 <br />
				<?php
				if(isset($datSocio['formIngresoCuota']['NOMARCHIVOSEPAXML']['valorCampo']) && !empty($datSocio['formIngresoCuota']['NOMARCHIVOSEPAXML']['valorCampo']))
				{echo	"<label>Nota: - Aquí NO se añaden los gastos del banco por las cuotas domiciliadas (por domiciliación o por devolución) que se incluirán en el siguiente apartado de DEVOLUCIÓN</label>";
				}
				?>
		<!--	Fin gastos cuota -------------------------------------------------->	
   ...
   //lo referente a fecha podría ser un requiere_once parValorFechas
  	...
  	echo comboLista($parValorMes,"datSocio[formIngresoCuota][FECHAPAGO][mes]",
		...
		<!-- *********** Fin datSocio[FECHAPAGO] *********** -->		
		 ...
		 <!-- Inicio radio button para incluir excluir en la próxima lista de cobro por banco la siguiente
				...
				<fieldset>
					...
					<p>	
					 ...
					 <label>Fecha de pago de cuota domiciliada por el banco </label>							
						...
						<br /><br />
					<?php	
/*							echo "<label>Cambiar estado actual de la cuota </label>"; 
							
						$parValorEstadoCuota = array("ABONADA"           =>"Abonada por el banco",																												   
																																			"NOABONADA-DEVUELTA"=>"Devuelta por el banco"																																		
																																		);																																																									
				
							echo comboLista($parValorEstadoCuota,"datSocio[formOrdenesCobro][ESTADOCUOTA]",
																						$datSocio['formOrdenesCobro']['ESTADOCUOTA']['valorCampo'],
																						$parValorEstadoCuota[$datSocio['formOrdenesCobro']['ESTADOCUOTA']['valorCampo']],"","");
*/																						
					?>
						<!--	Fin Estado actual de pago de orden cuota por el banco -------------------------->
						<br />			

						<label>Cantidad ORDENADA COBRAR al banco en la cuenta del socio/a</label>							
						<input type="text" readonly 
													class='mostrar'
				  ...
				  <label>Añadir <strong>GASTOS</strong> POR DEVOLUCIÓN de la cuota domiciliada por el banco</label>	
					 <input type="text"
										name="datSocio[formOrdenesCobro][ADD_IMPORTEGASTOSDEVOLUCION]"
										value='<?php 
										if (isset($datSocio['formOrdenesCobro']['ADD_IMPORTEGASTOSDEVOLUCION']['valorCampo']))
										{  echo $datSocio['formOrdenesCobro']['ADD_IMPORTEGASTOSDEVOLUCION']['valorCampo'];
										}
   							else
										{  echo "0.00";													
										}
					...
					<span class="error">
						<?php
						if (isset($datSocio['formOrdenesCobro']['ADD_IMPORTEGASTOSDEVOLUCION']['errorMensaje']))
						{echo $datSocio['formOrdenesCobro']['ADD_IMPORTEGASTOSDEVOLUCION']['errorMensaje'];}
						?>		
				 </span>
	   	<br />							
					<label>Nota: Si se necesitase corregir un error previo de anotación en gastos de DEVOLUCIÓN CUOTA DOMICILIADA, para disminuir la cantidad anotada, poner signo menos delante (-) delante de la cantidad a restar.</label>
					<!--<br />
			 	<label>Sólo admite correcciones en gastos de dovolución de cuota domiciliada, en el mismo año y mientas el estado actual de pago de la cuota sea "Devuelta, no abonada"</label>					
				 -->
      <br /><br />
					...
					<label>Motivo devolución</label>  
							<input type="text" 
														...
														value='<?php if (isset($datSocio['formOrdenesCobro']['MOTIVODEVOLUCION']['valorCampo']))
																											{  echo htmlspecialchars($datSocio['formOrdenesCobro']['MOTIVODEVOLUCION']['valorCampo'],ENT_QUOTES);}	
 																					?>'
														size="100"
														maxlength="500"
														/>		
	  ...
	  <?php if (isset($datSocio['formIngresoCuota']['ANIOCUOTA']['valorCampo']) 
		...
		class="textoAzul8Left" name="datSocio[formIngresoCuota][OBSERVACIONES]" rows="12" cols="80"><?php 
		  if (isset($datSocio['formIngresoCuota']['OBSERVACIONES']['valorCampo']))                    
			{echo htmlspecialchars($datSocio['formIngresoCuota']['OBSERVACIONES']['valorCampo'],ENT_QUOTES);}	
		?></textarea>
									<span class="error">
											<?php
											if (isset($datSocio['formIngresoCuota']['OBSERVACIONES']['errorMensaje'])) {
															echo $datSocio['formIngresoCuota']['OBSERVACIONES']['errorMensaje'];
											}
											?>
							</span>			
		</p>
