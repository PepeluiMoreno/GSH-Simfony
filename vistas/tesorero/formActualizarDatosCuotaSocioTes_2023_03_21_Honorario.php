<?php
/*----------------------------------------------------------------------------------------------------
FICHERO: formActualizarDatosCuotaSocioTes.php
VERSION: PHP 7.3.21

DESCRIPCION: Es el formulario para la actualización de la cantidad y tipo de cuota elegida por el socio, 
los datos bancarios, ORDENARCOBROBANCO y OBSERVACIONES y otros datos personales

LLAMADA: vCuerpoActualizarDatosCuotaSocioTes.php
y previamente desde cTesorero.php:actualizarDatosCuotaSocioTes()
      
OBSERVACIONES:
2023-02-05: Cambio fecha de nacimiento completa por sólo "año de nacimiento"  
-----------------------------------------------------------------------------------------------------*/
...
<div id="registro">
	...
	<?php //echo "datSocio['campoActualizar']: ";print_r($datSocio['campoActualizar']); ?>
	</span>
		...
		<!-- ****************** Inicio Mostrar Nombre y Usuario del socio ************* -->
	 <fieldset>
		<legend><strong>Nombre del socio/a</strong></legend>
		<p>
				<span class="mostrar">
      <?php 
									echo $datSocio['campoActualizar']['datosFormMiembro']['APE1']['valorCampo'];   
									if (isset($datSocio['campoActualizar']['datosFormMiembro']['APE2']['valorCampo']))
									{  echo " ".$datSocio['campoActualizar']['datosFormMiembro']['APE2']['valorCampo'];}
									echo ", ".$datSocio['campoActualizar']['datosFormMiembro']['NOM']['valorCampo'];
       ?>																		
      ...
      <br />
    <!--
	   <label>*Usuario/a</label> <!-- quitar de aquí y poner en mostrar 
	   <input type="text" readonly
	           ...
	           value='<?php /*if (isset($datSocio['campoActualizar']['datosFormUsuario']['USUARIO']['valorCampo']))
	                        {  echo $datSocio['campoActualizar']['datosFormUsuario']['USUARIO']['valorCampo'];}
	                  */?>'
	           size="35"
	           maxlength="50"
	    />
  ...
  <!-- ********************** Fin Mostrar Nombre y Usuario del socio ************ -->		

	
 <form name="actualizarSocio" method="post" class="linea" action="./index.php?controlador=cTesorero&amp;accion=actualizarDatosCuotaSocioTes">
  ...
  <!-- ****************** Inicio Campos hidden ************************************************** -->
   ...
   <!-- Inicio "campoHide"(incluye anteriorUSUARIO,anteriorEMAIL,anteriorCODPAISDOC,
			...
			<!-- Inicio de "privacidad" se pone siempre a SI para que la función de   validación 
						"validarCamposFormActualizarSocio()" para todos los casos socios, presidente, coordinador
						donde no se hace la pregunta privacidad 
			********************************************************************************************* -->	      
   ...
   <input type="hidden"	name="campoVerAnioActual[IMPORTECUOTAANIOEL]"
    ...
    />	
				<!-- Fin hidden: "campoVerAnioActual" también se usa para ver en pantalla ---------------_------->	
				
    <!-- Inicio hidden: "campoActualizar[datosFormCuotaSocio]" también se usa para ver en pantalla -->					
				<input type="hidden"
         	id="codUser"
          name="campoActualizar[datosFormUsuario][CODUSER]"
    ...
    />				
          ...
          value='<?php if (isset($datSocio['campoActualizar']['datosFormUsuario']['ESTADO']['valorCampo']))
                       {  echo $datSocio['campoActualizar']['datosFormUsuario']['ESTADO']['valorCampo'];}
                 ?>'
    />				
				<input type="hidden"
         	id="codSocio"
          name="campoActualizar[datosFormSocio][CODSOCIO]"
   ...
   <input type="hidden"	name="campoActualizar[datosFormCuotaSocio][IMPORTECUOTAANIOEL]"
    ...
    />	
   <input type="hidden" name="campoActualizar[datosFormMiembro][APE1]"
          value='<?php if (isset($datSocio['campoActualizar']['datosFormMiembro']['APE1']['valorCampo']))
                       {  echo " ".$datSocio['campoActualizar']['datosFormMiembro']['APE1']['valorCampo'];}?>'
   />	
   <input type="hidden" name="campoActualizar[datosFormMiembro][APE2]"
          value='<?php if (isset($datSocio['campoActualizar']['datosFormMiembro']['APE2']['valorCampo']))
                       {  echo " ".$datSocio['campoActualizar']['datosFormMiembro']['APE2']['valorCampo'];}?>'
   />	
   <input type="hidden" name="campoActualizar[datosFormMiembro][NOM]"
  ...
  <!-- ****************** Fin Campos hidden ***************************************************** -->
	 ...
	 <!-- ************************ Inicio cuotas del socio ****************************** -->	
	 <fieldset>
	  <legend><b>Datos de las cuotas del socio/a</b></legend>
		 <p>	
    <br />			
			 <!-- ************************** Inicio mostrar cuotas año actual ***************** -->			
				<?php 
					echo "<label>Cuota ".//$datSocio['campoVerAnioActual']['NOMBRECUOTA']['valorCampo'].
					...
					{echo "<span class='mostrar'>Devolución recibo de pago</span>";
					}				
					...
					echo "<label>&nbsp;&nbsp;&nbsp;Tipo cuota</label>";
					echo "<span class='mostrar'>".$datSocio['campoVerAnioActual']['CODCUOTA']."</span>"; 				
				?>
			 <!-- ****************************** Fin mostrar cuotas año actual ***************** -->			


			 <!-- ************************* Inicio actualizar importeCuotaSocio  ************** -->
			 <br /><br />
				<span class="comentario11">Ahora puedes modificar la cantidad elegida como cuota por el socio/a o el tipo de cuota, si el socio/a así te lo ha comunicado.		
																														<br />
																															Si en el presente año ya estuviese abonada la cuota, se anotará como nueva cuota para el próximo año. 
																															<br />Nota: para Honorarios aunque están exentos de pagar cuota (su cuota mínima es <strong> <?php echo ceil($datSocio['campoActualizar']['datosCuotasEL']['IMPORTECUOTAANIOELHonorario']).' € '; ?></strong>
																															) si quieren pueden domiciliar el pago de una cuota anual voluntaria o pagar una cuota voluntaria mediante Transferencia, PayPal, u otros medios.		
				</span>			

		  <br /><br />
					
				<label><strong>*Tipo cuota:</strong></label>	
						<span class="error">
						<?php
	    ...
	    <input type="radio"
	           ...
	           {  echo " checked";}
	           ?>
	    /><label>Parado/a o dificultades enconómicas&nbsp;(mínimo <?php echo ceil($datSocio['campoActualizar']['datosCuotasEL']['IMPORTECUOTAANIOELParado']).' €)'; ?></label>	
					
						<input type="radio"
	           name="campoActualizar[datosFormSocio][CODCUOTA]"
	           value='Honorario' 
						 <?php if ($datSocio['campoActualizar']['datosFormSocio']['CODCUOTA']['valorCampo']=='Honorario')
	           {  echo " checked";}
	           ?>
     /><label>Honorario&nbsp;(mínimo <?php echo ceil($datSocio['campoActualizar']['datosCuotasEL']['IMPORTECUOTAANIOELHonorario']).' € pero puede elegir domiciliar el pago de una cuota voluntaria)'; ?></label>									
		  <br />
					<span class="comentario11">
		 	...
		 	<label>Modificar la cuota total elegida por el socio/a para el año 
					<?php echo " <b>".$datSocio['campoActualizar']['datosFormCuotaSocio']['ANIOCUOTA']['valorCampo']." (cuota mínima + donación opcional)</b>";		
					?>
				</label>
     <input type="text"						       		        		        
	 ...
	 <!-- ************************ Fin cuotas del socio ********************************* -->	
		...
		<!-- ************************ Inicio actualizar cuenta banco *********************** -->	
		<fieldset>
	  <legend><strong>Modificar la cuenta del  banco del socio/a para el pago cuota anual</strong></legend>
	 	<p>
				...
				<label>Núm. cuenta bancaria <strong>IBAN</strong> (dos letras de país + número sin espacios)</label>
					<input type="text"
					...
					<br /><br />		
				<?php if (isset($datSocio['campoActualizar']['datosFormSocio']['SECUENCIAADEUDOSEPA']['valorCampo']) && 
													!empty($datSocio['campoActualizar']['datosFormSocio']['SECUENCIAADEUDOSEPA']['valorCampo']))
				{  echo "<span class='comentario11'>Tipo secuencia de pago a esta entidad </span>"; 
							echo "<span class='mostrar'>".$datSocio['campoActualizar']['datosFormSocio']['SECUENCIAADEUDOSEPA']['valorCampo']."</span>";
							echo "<span class='comentario11'>(RECUR: ya cobrado otras veces, FRST: no cobrado antes)</span>";	
							echo "<br /><br />";
				} 																							
				?>
	 ...
	 <!-- ************************ Fin actualizar cuenta banco ************************** -->		
		
				<!-- Inicio radio button para poder incluir excluir de la próxima lista de cobro por banco de la siguiente cuota -->
				<br />
				
	<fieldset>
	  <legend>Ordenar el cobro de la cuota en la cuenta del socio/a para la próxima remesa</legend>
		 <p>
				<label>*Incluir en la próxima lista de órdenes de cobro a los bancos (aún no se puede con c. extranjeras) del año 			
					...
					<span class="error">
						<?php
						if (isset($datSocio['campoActualizar']['datosFormCuotaSocio']['ORDENARCOBROBANCO']['errorMensaje']))
						{echo $datSocio['campoActualizar']['datosFormCuotaSocio']['ORDENARCOBROBANCO']['errorMensaje'];}
						?>
					</span>
			
					<input type="radio"
												name="campoActualizar[datosFormCuotaSocio][ORDENARCOBROBANCO]"
												value='SI' 
												<?php if ($datSocio['campoActualizar']['datosFormCuotaSocio']['ORDENARCOBROBANCO']['valorCampo']=='SI')
												{  echo " checked";}
												?>
					/>
			<label>SI&nbsp;&nbsp;&nbsp;&nbsp;</label>
				<input type="radio"
											name="campoActualizar[datosFormCuotaSocio][ORDENARCOBROBANCO]"
											value='NO'
						<?php if ($datSocio['campoActualizar']['datosFormCuotaSocio']['ORDENARCOBROBANCO']['valorCampo']=='NO')
											{  echo " checked";}
											?>						 
				/>
			<label>NO</label>		
  ...
  <!-- ********************** Inicio Agrupación territorial ************************** -->	
		<fieldset>
	  <legend>Agrupación territorial del socio/a</legend>
		 <p>
	 ...
	 <fieldset>
	  <legend>Datos personales</legend>
	   ...
	   <label>*País documento</label>
	    ...
	    <br />	
				<label>*Error en correo electrónico</label> 
					<?php
		  ...
		  <br /><br />				
				...
				<label>Profesión</label>
							<input type="text"
														...
														{  echo $datSocio['campoActualizar']['datosFormMiembro']['PROFESION']['valorCampo'];}
														?>'
														size="60"
														maxlength="255"
							/>	 
  ...
  <br />
	 ...
	 <!-- ***** Inicio ['COMENTARIOSOCIO'] y [OBSERVACIONES] para tabla MIEMBRO ********* -->				
		...
		<fieldset>
	  <legend><b>Comentarios del socio /a al darse de alta (visible para gestores/as)</b></legend>
		 <p>
				<textarea  id='COMENTARIOSOCIO' onKeyPress="limitarTextoArea(500,'COMENTARIOSOCIO');"	
				class="textoAzul8Left" name="campoActualizar[datosFormMiembro][COMENTARIOSOCIO]" rows="4" cols="80"><?php 
				...
				?></textarea> 
									<span class="error">
												<?php
												if (isset($datSocio['campoActualizar']['datosFormMiembro']['COMENTARIOSOCIO']['errorMensaje'])) {
																echo $datSocio['campoActualizar']['datosFormMiembro']['COMENTARIOSOCIO']['errorMensaje'];
												}
												?>
									</span>			
	 	</p>
	 ...
	 <fieldset>
	  <legend><strong>Observaciones de gestión de socios/as (visible para gestores/as)</strong></legend>
	 	<p>
  ...
  <!-- *** Inicio datos ['OBSERVACIONES'] en tabla "CUOTAANIOSOCIO" por Tesorero ****** -->			
	 <fieldset>
	  <legend><strong>Observaciones de tesorería </strong> ( sólo visibles para tesorería, se anotará en datos de la cuota de 
	  <?php if (isset($datSocio['campoVerAnioActual']['ESTADOCUOTA']) 
				          && $datSocio['campoVerAnioActual']['ESTADOCUOTA'] =='ABONADA'
				        )
            { echo  ++$datSocio['campoVerAnioActual']['ANIOCUOTA']." )";}
												else
												{ echo  $datSocio['campoVerAnioActual']['ANIOCUOTA']." )";}
		 ...
		 <p>
				...
				class="textoAzul8Left" name="campoActualizar[datosFormCuotaSocio][OBSERVACIONES]" rows="12" cols="80"><?php 
						if (isset($datSocio['campoActualizar']['datosFormCuotaSocio']['OBSERVACIONES']['valorCampo']))                    
					{echo htmlspecialchars($datSocio['campoActualizar']['datosFormCuotaSocio']['OBSERVACIONES']['valorCampo']);}
				?></textarea>
									<span class="error">
													<?php
													if (isset($datSocio['campoActualizar']['datosFormCuotaSocio']['OBSERVACIONES']['errorMensaje'])) {
																	echo $datSocio['campoActualizar']['datosFormCuotaSocio']['OBSERVACIONES']['errorMensaje'];
													}
													?>
									</span>					
			</p>
	 </fieldset>
		<!-- *** Fin datos ['OBSERVACIONES'] en tabla "CUOTAANIOSOCIO" por Tesorero ********* -->		
		
	 <br />
		
		<!-- ********** Inicio Botones de formActualizarDatosCuotaSocioTes ****************** -->  
		<span class="error">
									<?php
									if (isset($datSocio['codError']) && $datSocio['codError'] !== '00000') {
													echo "<b>ERROR AL INTRODUCIR LOS DATOS: revisa los datos con comentarios de error en color rojo</b>";
 	...
 	<br />					
		...
		<!-- </div>		-->										
		<!-- *********** Fin Botones de formActualizarDatosCuotaSocioTes ********************* -->
...
<!-- ********** Fin del formulario formActualizarDatosCuotaSocioTes ******************** -->	
